...
function CatalogEntry(record: CatalogEntryProps) {
  return (
    <div className="border border-slabs-border bg-slabs-surface cursor-default hover:-translate-y-[3px] hover:shadow-[0_4px_16px_rgba(20,19,19,0.08)] transition-all duration-[200ms] ease-analog flex flex-col group/card">
      ...
      {/* Spec block */}
      <div className="grid grid-cols-[56px_1fr] px-3 pt-2.5 pb-3 border-t border-slabs-divider text-left flex-1 bg-slabs-surface">
        {/* Left column */}
        <div className="border-r border-slabs-divider pr-2 flex flex-col justify-between">
          <div>
            <span className="block font-display text-[11px] font-normal tracking-[0.04em] uppercase text-slabs-text-faint leading-tight">
              {record.index}
            </span>
            <span className="block font-display text-[11px] font-normal tracking-[0.04em] uppercase text-slabs-text-faint leading-tight">
              {record.genre}
        ...
        <div className="pl-2.5 flex flex-col justify-between h-full">
          <div>
            <span className="block font-display text-[11px] font-normal tracking-[0.04em] uppercase text-slabs-text leading-tight">
              {record.artist}
            </span>
            <span className="block font-display text-[11px] font-normal tracking-[0.04em] uppercase text-slabs-text-muted leading-tight">
              {record.title}
            </span>
            <span className="block font-display text-[11px] font-normal tracking-[0.04em] uppercase text-slabs-text-muted leading-tight">
    ...
    <section className="bg-paper py-[clamp(4rem,8vw,8rem)]">
      <div className="w-full max-w-content px-[clamp(1.5rem,5vw,5rem)] mx-auto">
        ...
        <header className="mb-10 text-left">
          <h2 className="font-display text-ui-over font-semibold uppercase tracking-[0.12em] text-slabs-text-faint mb-3">
            NEW ARRIVALS
          </h2>
          <p className="font-serif italic text-display-sm font-normal text-slabs-text m-0">
            Just dropped.
          </p>
        </header>
